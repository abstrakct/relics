...
impl<'de, E: EventConvertible> Deserialize<'de> for KeyBindings<E> {
    ...
    where
        D: Deserializer<'de>,
    {
        let parsed_map = HashMap::<GameState, HashMap<String, serde_json::Value>>::deserialize(deserializer)?;
        ...
        let keybindings = parsed_map
            ...
            .map(|(mode, inner_map)| {
                let converted_inner_map = inner_map
                    .into_iter()
                    .filter_map(|(key_str, event_value)| {
                        let key_events = parse_key_sequence(&key_str).ok()?;
                        let event = E::from_value(event_value)?;
