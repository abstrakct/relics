...
use crossterm::{
    ...
    event::{
        ...
        KeyEvent, KeyEventKind, KeyboardEnhancementFlags, MouseEvent, PopKeyboardEnhancementFlags,
...
impl Tui {
    ...
    pub fn start(&mut self) {
        ...
        self.task = tokio::spawn(async move {
            let mut event_stream_reader = crossterm::event::EventStream::new();
            ...
            loop {
                ...
                let crossterm_event = event_stream_reader.next().fuse();
                tokio::select! {
                        ...
                        match maybe_event {
                          ...
                          CrosstermEvent::Key(key) => {
                            if key.kind == KeyEventKind::Repeat {
                              log::debug!("Repeated key press detected!");
                            } else if key.kind == KeyEventKind::Press {
                              log::debug!("Key press detected!");
                              _event_tx.send(Event::Key(key)).unwrap();
                            } else if key.kind == KeyEventKind::Release {
                              log::debug!("Key release detected!");
                            }
                          },
    ...
    pub fn enter(&mut self) -> Result<()> {
        ...
        if supports_keyboard_enhancement {
            log::debug!("Keyboard enhancements are supported");
            crossterm::queue!(
                stdout,
                PushKeyboardEnhancementFlags(
    ...
    pub fn exit(&mut self) -> Result<()> {
        log::debug!("Entering Tui::exit");
        self.stop()?;
        if crossterm::terminal::is_raw_mode_enabled()? {
            self.flush()?;
            let supports_keyboard_enhancement = matches!(crossterm::terminal::supports_keyboard_enhancement(), Ok(true));
            if supports_keyboard_enhancement {
                crossterm::execute!(io(), PopKeyboardEnhancementFlags)?;
            }
            if self.paste {
